...
def difference_of_gaussians(img_raw: np.ndarray, sigma_c, sigma_e, sigma_m, sigma_a, p, phi, epsilon) -> np.ndarray:
    ...
    del img_raw
    print("grayscale!")
    J11, J12, J22 = anisotropic_filters.structure_tensor(img, sigma_smooth=sigma_c)

    v1, v2, _1, _2 = anisotropic_filters.eigenvectors_from_structure_tensor(J11, J12, J22)
    ...
    Xdog[~bright_mask] = 1 + np.tanh(phi * (Xdog[~bright_mask] - epsilon_threshold))
